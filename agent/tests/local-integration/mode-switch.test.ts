...
describe("batch device plan snapshot and mode switching", () => {
  ...
  it("fails closed for unsupported decision policy even when modes match", async () => {
    const { base, cookie, filename, store, userId } = await startApi();
    const created = await createBatch(base, cookie);
    const advance = await apiRequest(base, `/api/batches/${created.batch.id}/advance`, {
      method: "POST",
      ...
      body: JSON.stringify({
        expectedRevision: 0,
        idempotencyKey: "unsupported-policy-advance",
        observation: { effectiveHeads: 20, creepGrade: "none", diarrheaGrade: "none", actualPowderGrams: 0 },
      }),
    });
    expect(advance.status).toBe(200);
    const dayActual = await apiRequest(base, `/api/batches/${created.batch.id}/records`, {
      ...
      body: JSON.stringify({
        expectedRevision: 1,
        idempotencyKey: "unsupported-policy-day-zero",
        observation: { effectiveHeads: 20, creepGrade: "none", diarrheaGrade: "none", actualPowderGrams: 0 },
      }),
    });
    expect(dayActual.status).toBe(200);
    ...
    const unsupportedDecision = {
      ...
      evidence: {
        ...
        inputs: {
          ...canonical.decision.evidence.inputs,
          decisionPolicyVersion: "future-x",
        },
      },
    };
    const database = new DatabaseSync(filename);
    ...
    `).run(
      ...
      "unsupported-policy",
      created.batch.id,
      unsupportedDecision.revision,
      unsupportedDecision.dateLocal,
      unsupportedDecision.evidence.sopVersion,
      unsupportedDecision.evidence.modelVersion,
      unsupportedDecision.evidence.calculationDate,
