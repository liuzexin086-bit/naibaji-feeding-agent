...
- 最近三次观察中至少两次达到同一档或更高才形成持续教槽状态，并从次日冻结 `controlStartDay`；单次非 `none` 不触发控奶。
- 控奶餐次只减不增、每天最多减一次、最高为 10 次；低/中/高/极好的最低餐次分别为 `9/8/6/4`，不允许 UI 自行计算。
...
- 单次非“无”教槽不触发控奶；持续教槽成立后从下一日开始，餐次按档位下限逐日最多减少一次且绝不超过 10 次。
