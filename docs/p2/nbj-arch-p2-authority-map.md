...
| Schema version/migration history | `MIGRATION_VERSION=12`; P2-3A candidate uses an ordered checksum ledger plus a provenance-only archive of the old two-column rows | Ordered checksum migration ledger in `packages/persistence/migrations` plus SQLite `schema_migrations` | Migration runner only | Audited v12 is imported as the first trusted identity; historical v3-v11 rows are preserved without invented checksums | Runtime ad-hoc schema repair |
