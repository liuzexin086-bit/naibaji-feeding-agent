...
| Schema version/migration history | P2-3A sealed v12; P2-3B candidate registers forward v13 repair and retains a provenance-only archive of the old two-column rows | Ordered checksum migration ledger in `packages/persistence/migrations` plus SQLite `schema_migrations` | Migration runner only | Audited v12 is imported as the first trusted identity; historical v3-v11 rows are preserved without invented checksums | Runtime ad-hoc schema repair |
