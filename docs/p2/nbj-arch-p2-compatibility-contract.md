...
|---|---|---|---|---|
| `P2-ED-001` | Chat `observation.diarrheaGrade="none"` was normalized to an omitted grade, while the core API/store contract preserves explicit `none`. | One typed observation contract preserves `observed:none` distinctly from `not_observed` through chat, API, Domain, and persistence; alternatively remove chat observations from the authoritative input contract. | `IMPLEMENTED / INDEPENDENT ACCEPTANCE PENDING`: the chat projection preserves explicit `none`, keeps omission absent, accepts legacy numeric-string/empty-string powder forms, rejects invalid selected values, and exposes no application/device claim. Current-turn explicit-none closure is covered in preview and sync paths. | `agent/tests/domain/chat-server-http.test.ts` exercises the real `/api/feeding-agent/chat` HTTP entry with the complete UI payload, omission/explicit-none context distinction, and 400 fail-closed cases; `agent/tests/local-integration/local-tools.test.ts` and `agent/tests/operations/observation-feedback.test.ts` cover prior-moderate closure. Persistence wiring remains a later stage; independent acceptance is still required. |
