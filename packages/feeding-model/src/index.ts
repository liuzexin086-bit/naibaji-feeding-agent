...
/** 体重标准（对应日龄的最低合格体重） */
export const WEIGHT_STANDARD: Record<number, number> = {
...
export const CREEP_GRADE_VALUES: Readonly<Record<string, number>> = Object.freeze({
  none: 0,
  low: 10,
  medium: 45,
  high: 80,
...
export const CREEP_GRADE_ORDER: readonly string[] = Object.freeze(['none', 'low', 'medium', 'high', 'excellent'])
export const CREEP_MEAL_FLOORS: Readonly<Record<string, number>> = Object.freeze({
  none: 10,
  low: 9,
  medium: 8,
  high: 6,
...
function creepValueFromRecord(record: any): number | null {
...
function creepGradeFromRecord(record: any): string {
  const label = record && (record.creepGrade ?? record.creep_grade)
...
function sustainedCreepGrade(grades: string[]): string {
...
/** 验证批次参数，不通过则抛错 */
export function validateBatchConfig(opts: PlanOptions): void {
  const errors: string[] = []
...
export function getDailyCapacity(weightKg: number): number {
  return weightKg * INTERNAL.stomachMlPerKg * INTERNAL.safetyFactor * 12 / INTERNAL.dilution
}

export function getRampFactor(dayIndex: number): number {
...
/** 奶粉 → 增重 (kg) */
export function milkToGain(milkG: number): number {
  return milkG / (INTERNAL.fcr * 1000)
}

/** 从抽样数组推算全群加权平均重 */
function computeWeightedAvg(weighSample: { avgKg: number; headCount: number }[]): number | null {
...
export function generatePlan(opts: PlanOptions): FeedingPlan {
  // 输入验证
  validateBatchConfig(opts)

  const startAge = opts.startAge
  const endAge = opts.endAge
  const startWeight = opts.startWeight as number
...
export function computeControlPlan(plan: FeedingPlan, records: DailyRecord[], controlStartDay?: number | undefined): ControlPlan {
  ...
  const creepMap: Record<number, number> = {}
  const creepGradeMap: Record<number, string> = {}
  const committed = new Set<number>()
  const committedPlans: Record<number, { perPig: number; total: number; feedTimes: number }> = {}
  for (let i = 0; i < records.length; i++) {
    ...
    if (r.planPerPigAtCommit != null) {
      committedPlans[r.dayAge] = {
        ...
        total: r.planTotalAtCommit as number,
  ...
  let previousCommittedCount: number | null = null
  ...
  const creepGradeRolling: string[] = []
  let currentCount = 10
...
/** 批次结束时输出训练数据 */
export function exportTrainingData(batchInfo: BatchInfo): TrainingExport {
